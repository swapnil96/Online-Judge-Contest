...
void find()
{
	int i, n, size_p, size_m;
	scanf("%d", &n);
	long long int temp;
	InfInt temp_m, temp_p, ans, ans1, ans2;;
	...
	sort(minus.begin(), minus.end(), greater<long long int>());
	ans1 = temp_p*size_p + temp_m;
	for (i = 0; i < size_m; i++)
	{
		temp_p += minus[i];
		temp_m -= minus[i];
		size_p += 1;
		ans2 = temp_p*size_p + temp_m;
		if (ans2 > ans1)
			ans1 = ans2;
	}
	cout << ans1 << endl;
